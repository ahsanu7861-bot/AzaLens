-- Migration 009: correct the not-yet-instantiated broker cost-schedule contract.
-- Migration 008 is already applied and remains immutable. This correction is
-- deliberately permitted only while no schedule evidence exists.
...
do $$ begin
 ...
 or exists(select 1 from public.broker_cost_schedule_components) then
  raise exception using errcode='55000',message='migration 009 refused: broker cost schedule evidence already exists';
 end if;
end $$;
...
alter table public.broker_cost_schedule_versions
  add constraint broker_cost_schedule_versions_broker_legal_entity_check
    check (broker_legal_entity = 'Saxo Bank'),
  add constraint broker_cost_schedule_versions_evidence_limitations_check
    check (evidence_limitations = E'The broker name Saxo Bank is printed on the owner''s Account Statement, Cost Overview Report and Personal Information export. The account agreement''s contracting-entity clause has not been verified. No client identifier or other personal detail is stored.\nThe approved allowance formula includes a fixed 0.20 USD term per modeled exit and is subject to a 0.50 USD floor. It is a predictive planning input, not proven coverage or an upper bound for broker rounding.\nThe allowance does not reproduce Saxo''s actual charges. Saxo''s rounding, minimums, multi-fill execution, per-fill charges and venue-routing treatment remain unproven and may produce different costs.');
...
begin
 ...
 v_fp:=encode(extensions.digest(convert_to(jsonb_build_object('contract',2,'broker_legal_entity','Saxo Bank',
  'finra_source_effective_from',date '2026-01-01','effective_from',p_effective_from,
 ...
 values(v_user,v_version,'Saxo Bank','Classic','USD','LONG_US_LISTED_CASH_EQUITY',
  ...
  100000,10000,p_evidence_reference,E'The broker name Saxo Bank is printed on the owner''s Account Statement, Cost Overview Report and Personal Information export. The account agreement''s contracting-entity clause has not been verified. No client identifier or other personal detail is stored.\nThe approved allowance formula includes a fixed 0.20 USD term per modeled exit and is subject to a 0.50 USD floor. It is a predictive planning input, not proven coverage or an upper bound for broker rounding.\nThe allowance does not reproduce Saxo''s actual charges. Saxo''s rounding, minimums, multi-fill execution, per-fill charges and venue-routing treatment remain unproven and may produce different costs.',p_evidence_captured_at,p_idempotency_key,v_fp) returning id into v_id;
 ...
 (v_id,v_user,'CLOSE_COMMISSION','BROKER_PROVEN','DERIVED_CHARGE',.0008,null,1,null,null,'Saxo Bank',p_broker_source_reference,p_effective_from,'Broker-proven commission only; the broker name is copied from account documents and the contracting-entity clause remains unverified. No regulatory pass-through or routing behavior is implied.'),
 ...
 (v_id,v_user,'FINRA_TAF_REFERENCE','REGULATOR_PUBLISHED','REFERENCE_ONLY',null,.000195,null,9.79,null,'FINRA',p_finra_source_reference,date '2026-01-01','Reference only. FINRA paused TAF assessment for transactions from 2026-10-01 through 2026-12-31. The rate remains an intentionally conservative input to the founder-approved predictive allowance; it is not represented as a FINRA assessment billed by Saxo, and its cap does not modify the allowance formula.'),
 (v_id,v_user,'REGULATORY_ALLOWANCE','FOUNDER_APPROVED_ALLOWANCE','FIXED_OR_FORMULA_ALLOWANCE',.0000206,.000195,.50,null,.20,'AzaLens founder',p_allowance_approval_reference,p_effective_from,'Approved conservative planning buffer, including during the documented 2026 FINRA TAF pause; not a FINRA assessment, measured Saxo charge or proven upper bound. Saxo rounding, execution splitting and venue routing remain unproven.');
