...
async function main(){
  ...
  const provenance=[
    {capability:"QUOTE",provider:"Finnhub",state:"REALTIME_CONSOLIDATION_UNVERIFIED",underlying_state:"REALTIME_CONSOLIDATION_UNVERIFIED",source_timestamp:"2026-09-03T00:00:00Z",retrieval_timestamp:"2026-09-03T00:00:01Z",cache_state:"MISS",cache_age_seconds:null,interval:null,display_entitlement:"PRIVATE_PERSONAL_OWNER_ONLY",broker_verification_required:true,limitations:["Consolidation unverified"]},
    {capability:"HISTORY",provider:"TwelveData",state:"EOD_CONSOLIDATED",underlying_state:"EOD_CONSOLIDATED",source_timestamp:"2026-09-02T00:00:00Z",retrieval_timestamp:"2026-09-03T00:00:01Z",cache_state:"MISS",cache_age_seconds:null,interval:"1day",display_entitlement:"NON_DISPLAY_DERIVED_ANALYTICS_ONLY",broker_verification_required:true,limitations:["Non-reconstructive analytics only"]}
  ];
  ...
  try {
    ...
    const stringifiedBody=createBody(uuid(),"JSON"); stringifiedBody.p_thesis_text='{"renamed":"987654.12345678"}';
    const stringified=await create(stringifiedBody);
    assert.ok(!stringified.ok); assert.equal(sql(`select count(*) from public.outcome_decision_snapshots where symbol in ('RAW','NEST','JSON')`),"0");
