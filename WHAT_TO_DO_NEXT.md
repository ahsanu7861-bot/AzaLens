...
| 2.17 | **The public landing page brands deterministic computation as AI.** Observed live on `https://www.azalens.com` after the PR 2 merge. Exact strings and their sources: **"AI Stock Intelligence"** — `Hero` (`frontend/src/components/landing/Hero.tsx`, observed line 8) and `Navbar` (`frontend/src/components/landing/Navbar.tsx`, observed line 16); **"AI-powered analysis for listed stocks worldwide…"** — `Hero` (observed line 23); **"TRANSPARENT AI ANALYSIS"** — `ProductPreview` (`frontend/src/components/landing/ProductPreview.tsx`, observed line 9). All three components are mounted by `frontend/src/pages/LandingPage.tsx`, so every string is live to the public. **This is the same class of defect PR 1B corrected inside the application** — PR 1B renamed `AIVerdictCard`→`VerdictCard`, `AIExplanation`→`EvidenceSummary`, deleted `AIReasoning.tsx`/`AIVerdict.tsx` and renamed the "AI thesis" tab to "Thesis" — but the marketing surface was never included, so the wording PR 1B removed from the product is still what a first-time visitor reads. **It directly contradicts the ratified no-LLM decision** (`docs/LLM_DECISION_V1.md` §8 item 4: "Introduce no 'AI' wording anywhere; do not brand deterministic computation as AI"), approved 2026-08-11 in commit `7a9c4c6`. AzaLens v1 contains no model, no SDK and no model key; the analysis is deterministic string templates and arithmetic. Claiming otherwise in public marketing is therefore **a Rule 7 truthfulness defect, not a copy preference**. Correcting it changes user-visible copy and may require new Linux visual baselines, so it belongs with the landing-page work in items 2.15–2.16 and must not be done piecemeal. *Adjacent, lower priority:* `frontend/src/components/analysis/TradePlan.tsx` (observed line 43) still contains "AI Trade Plan"; it is exported from `components/analysis/index.ts` but mounted nowhere, so it is unrendered dead code — fold into the dead-barrel-export cleanup rather than this item. Observed at merge SHA `31fafd5`; **line references will drift — use the file and component names as the locators** | Proven live, deliberately deferred | 2, 7 | None |

| 2.18 | **`computeLegacyAgreementConfidenceForRisk` is temporary compatibility debt.** `backend/analysis/risk/legacyAgreementCompat.js` reproduces the pre-PR-3 agreement percentage from the nine legacy readings, consumed only by the evidence-confirmation bucket inside `analyzeRisk`. It exists **only** to preserve risk behaviour while Evidence Agreement changed from a percentage to independent family counts; without it, removing the field would have defaulted every analysis to the lowest bucket and silently moved published risk levels. It is never serialized, never rendered, never exposed in frontend types, and never described to a user as confidence. **Removal gate:** it may remain only until a dedicated risk-evidence contract is approved with documented semantics, pinned scenarios and differential analysis. That follow-up must replace the shim or explicitly authorise its continued use. **No further evidence-model expansion may proceed while this compatibility debt remains unresolved.** | Introduced by PR 3, deliberately temporary | 7, 8 | None |
